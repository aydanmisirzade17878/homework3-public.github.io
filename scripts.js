...
function DrawStar(context) {
    // The initial position is slightly below the center of the canvas
    x = context.canvas.width / 2;
    y = context.canvas.height / 2 + 200;
    ...
    for (let i = 0; i < 5; i++) {
        moveForward(300, context); // Draw the outer edge
        context.stroke();
        turnRight(144); // Turn to create the star's points
    }
